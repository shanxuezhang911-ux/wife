...
/* 30 */
/*!***************************************************!*\
  !*** /Users/centurygame/wife/frontend/pages.json ***!
  \***************************************************/
/*! no static exports found */
/***/ (function(module, exports) {
...
/* 43 */
/*!********************************************************!*\
  !*** /Users/centurygame/wife/frontend/utils/config.js ***!
  \********************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
var SERVER_IP = '10.0.49.94';
var _default = {
  // ==================== 服务器地址 ====================
  API_BASE: "http://".concat(SERVER_IP, ":8091"),
  WS_URL: "ws://".concat(SERVER_IP, ":8091/ws/doubao"),
  // ==================== 音频配置（纯本地） ====================
  AUDIO: {
...
var callbacks = {
  ...
  // 会话启动
  onAudioData: null,
  // 收到AI音频数据 (ArrayBuffer)
  onASRText: null,
  // 收到ASR识别文本 ({text, isInterim})
  onASRStart: null,
  // 检测到用户开始说话
  onASREnd: null,
  // 用户说完了
...
function connect(cbs, deviceId) {
  ...
  ws = uni.connectSocket({
    ...
    success: function success() {
      return console.log('[Doubao] 小程序WS发起连接');
    },
    fail: function fail(err) {
...
function onOpen() {
  ...
  globalSequence = 0;
  // 发送 StartConnection
  sendBinary((0, _doubaoProtocol.encodeTextEvent)(_doubaoProtocol.EVENT.StartConnection, {}, null));
}
...
function onBinaryMessage(data) {
  var _sessionConfig, _sessionConfig2;
  var frame = (0, _doubaoProtocol.decodeFrame)(data);
  if (frame.isError) {
  ...
  switch (frame.eventId) {
    // ---- Connection ----
    case _doubaoProtocol.SERVER_EVENT.ConnectionStarted:
      console.log('╔══════════════════════════════╗');
      ...
      startSession();
      break;
    case _doubaoProtocol.SERVER_EVENT.ConnectionFailed:
      console.error('[连接] 失败:', frame.payload);
      callbacks.onError && callbacks.onError(frame.payload);
      break;

    // ---- Session ----
    case _doubaoProtocol.SERVER_EVENT.SessionStarted:
      console.log('[会话] 已启动, dialogId:', frame.payload && frame.payload.dialog_id);
      console.log('[会话] 模型:', (_sessionConfig = sessionConfig) === null || _sessionConfig === void 0 ? void 0 : _sessionConfig.modelVersion, '音色:', (_sessionConfig2 = sessionConfig) === null || _sessionConfig2 === void 0 ? void 0 : _sessionConfig2.speaker);
      isSessionActive = true;
      callbacks.onSessionStarted && callbacks.onSessionStarted(frame.payload);
      break;
    case _doubaoProtocol.SERVER_EVENT.SessionFinished:
      console.log('[会话] 已结束');
      isSessionActive = false;
      break;
    case _doubaoProtocol.SERVER_EVENT.SessionFailed:
      console.error('[会话] 失败:', frame.payload);
      callbacks.onError && callbacks.onError(frame.payload);
      break;

    // ---- ASR ----
    case _doubaoProtocol.SERVER_EVENT.ASRInfo:
      currentASRText = '';
      console.log('────────────────────────────────');
      console.log('[ASR] 检测到用户开始说话...');
      callbacks.onASRStart && callbacks.onASRStart();
      break;
    case _doubaoProtocol.SERVER_EVENT.ASRResponse:
      if (frame.payload && frame.payload.results) {
        frame.payload.results.forEach(function (r) {
          var text = r.text || '';
          var interim = r.is_interim || false;
          if (interim) {
          ...
          callbacks.onASRText && callbacks.onASRText({
            text: text,
            isInterim: interim
          });
        });
      ...
      break;
    case _doubaoProtocol.SERVER_EVENT.ASREnded:
    ...
    // ---- TTS ----
    case _doubaoProtocol.SERVER_EVENT.TTSSentenceStart:
    ...
    // ---- Chat ----
    case _doubaoProtocol.SERVER_EVENT.ChatResponse:
      ...
      break;
    case _doubaoProtocol.SERVER_EVENT.ChatEnded:
      console.log('┌──────── AI完整回复 ────────┐');
    ...
    // ---- 其他 ----
    case _doubaoProtocol.SERVER_EVENT.ConfigUpdated:
      console.log('[配置] 已更新');
      break;
    case _doubaoProtocol.SERVER_EVENT.UsageResponse:
      if (frame.payload && frame.payload.usage) {
        var u = frame.payload.usage;
        console.log('[用量] 输入文本tokens:', u.input_text_tokens, '输入音频tokens:', u.input_audio_tokens, '输出文本tokens:', u.output_text_tokens, '输出音频tokens:', u.output_audio_tokens);
      }
      break;
    case _doubaoProtocol.SERVER_EVENT.ChatTextQueryConfirmed:
      console.log('[Chat] 文本Query已确认');
      break;
    case _doubaoProtocol.SERVER_EVENT.DialogCommonError:
      console.error('[错误] 对话异常:', JSON.stringify(frame.payload));
      callbacks.onError && callbacks.onError(frame.payload);
      break;
    default:
      if (frame.eventId > 0) {
        console.log('[Doubao] 未处理事件:', (0, _doubaoProtocol.getEventName)(frame.eventId), frame.payload);
      }
...
function startSession() {
  ...
  sendBinary((0, _doubaoProtocol.encodeTextEvent)(_doubaoProtocol.EVENT.StartSession, payload, sessionId));
}
...
function sendTextQuery(text) {
  ...
  sendBinary((0, _doubaoProtocol.encodeTextEvent)(_doubaoProtocol.EVENT.ChatTextQuery, {
    content: text
  }, sessionId));
}
...
function finishSession() {
  ...
  sendBinary((0, _doubaoProtocol.encodeTextEvent)(_doubaoProtocol.EVENT.FinishSession, {}, sessionId));
  isSessionActive = false;
}
...
exports.SERVER_EVENT = exports.EVENT = void 0;
exports.decodeFrame = decodeFrame;
exports.encodeTextEvent = encodeTextEvent;
...
function getTextDecoder() {
  ...
  return {
    decode: function decode(uint8arr) {
      var bytes = uint8arr instanceof Uint8Array ? uint8arr : new Uint8Array(uint8arr);
      var str = '';
      for (var i = 0; i < bytes.length;) {
        var b = bytes[i];
        if (b < 0x80) {
          ...
          var surr = cp - 0x10000;
          str += String.fromCharCode(0xD800 + (surr >> 10), 0xDC00 + (surr & 0x3FF));
          i += 4;
        } else {
          i++;
        }
...
// Message Types
var MSG_FULL_CLIENT_REQUEST = 1; // 0x1 客户端文本事件
var MSG_FULL_SERVER_RESPONSE = 9; // 0x9 服务端文本事件
var MSG_AUDIO_ONLY_RESPONSE = 11; // 0xB 服务端音频数据
var MSG_ERROR = 15; // 0xF 错误

// Flags
var FLAG_EVENT = 4; // 携带 event ID
...
var EVENT_NAMES = {};
Object.entries(SERVER_EVENT).forEach(function (_ref) {
  var _ref2 = (0, _slicedToArray2.default)(_ref, 2),
    k = _ref2[0],
    v = _ref2[1];
  EVENT_NAMES[v] = k;
});
Object.entries(EVENT).forEach(function (_ref3) {
...
function getEventName(id) {
  return EVENT_NAMES[id] || "Unknown(".concat(id, ")");
}
 ...
 * @returns {ArrayBuffer}
 */
function encodeTextEvent(eventId, jsonPayload, sessionId) {
  var payloadStr = jsonPayload !== null ? JSON.stringify(jsonPayload) : '{}';
  var payloadBytes = textEncoder.encode(payloadStr);

  // 判断是否是 Connect 级事件
  var isConnectEvent = eventId === EVENT.StartConnection || eventId === EVENT.FinishConnection;

  // optional 字段: event(4B) + [sessionIdSize(4B) + sessionId(NB)]
  var optionalSize = 4; // event 固定4字节
  var sessionIdBytes = null;
  ...
  var totalSize = 4 + optionalSize + 4 + payloadBytes.length;
...
function decodeFrame(data) {
  ...
  var messageType = byte1 >> 4 & 0x0F;
  var flags = byte1 & 0x0F;
  var byte2 = view.getUint8(2);
  var serialization = byte2 >> 4 & 0x0F;

  // 仅非音频帧打印调试（音频帧太多会刷屏）
  if (messageType !== MSG_AUDIO_ONLY_RESPONSE) {
    var headerHex = Array.from(bytes.slice(0, Math.min(12, bytes.length))).map(function (b) {
      return b.toString(16).padStart(2, '0');
    }).join(' ');
    console.log('[Protocol] 帧: ' + data.byteLength + 'B, type=0x' + messageType.toString(16) + ', flags=0x' + flags.toString(16) + ' [' + headerHex + ']');
  }
  var isAudio = messageType === MSG_AUDIO_ONLY_RESPONSE;
  var isError = messageType === MSG_ERROR;
  var isServerResponse = messageType === MSG_FULL_SERVER_RESPONSE;
  var offset = 4;
  var eventId = 0;
  var errorCode = 0;
  ...
  if (isError) {
    ...
    if (offset + 4 <= data.byteLength) {
      var payloadSize = view.getUint32(offset);
      offset += 4;
      if (payloadSize > 0 && offset + payloadSize <= data.byteLength) {
        try {
          var text = textDecoder.decode(new Uint8Array(data, offset, payloadSize));
  ...
  // event
  var hasEvent = (flags & FLAG_EVENT) !== 0;
  if (hasEvent) {
    if (offset + 4 <= data.byteLength) {
  ...
  if (offset + 4 <= data.byteLength) {
    var possibleSize = view.getUint32(offset);
  ...
  var payload = null;
  if (offset + 4 <= data.byteLength) {
    var _payloadSize = view.getUint32(offset);
    offset += 4;
    if (_payloadSize > 0 && offset + _payloadSize <= data.byteLength) {
      if (isAudio) {
        payload = data.slice(offset, offset + _payloadSize);
      } else {
        try {
          var _text = textDecoder.decode(new Uint8Array(data, offset, _payloadSize));
          payload = JSON.parse(_text);
        } catch (e) {
          payload = textDecoder.decode(new Uint8Array(data, offset, _payloadSize));
        }
      }
    }
  }
...
exports.archiveCurrentRound = archiveCurrentRound;
exports.clearHistory = clearHistory;
exports.collectFrame = collectFrame;
exports.destroyPlayer = destroyPlayer;
exports.feedAudio = feedAudio;
exports.flushSentence = flushSentence;
exports.getBufferDelay = getBufferDelay;
exports.getHistory = getHistory;
exports.initPlayer = initPlayer;
exports.replayRandom = replayRandom;
exports.stopPlayback = stopPlayback;
exports.stopReplay = stopReplay;
exports.waitPlaybackEnd = waitPlaybackEnd;
/**
 * 音频播放模块
...
// ==================== 音频历史（Max复播） ====================
var currentRoundFrames = []; // 当前轮收集的PCM帧
var audioHistory = []; // 归档的历史音频 ArrayBuffer[]
var replaySource = null; // 复播中的AudioBufferSourceNode
var replayTimer = null; // 复播衰减定时器

var wxAudioContext = null;
var wxGainNode = null;
var wxNextStartTime = 0;
var wxVolumeTimer = null;
var wxIsPlaying = false;

/**
 * 初始化播放器
 * @param {function} onVolume - (volume: 0-100) 音量变化回调
 */
function initPlayer(onVolume) {
 ...
 * 等待当前缓冲音频全部播完后回调
 */
function waitPlaybackEnd(callback) {
  if (!wxAudioContext || wxNextStartTime <= wxAudioContext.currentTime) {
    callback();
    return;
  }
  var checkWx = function checkWx() {
    if (!wxAudioContext || wxNextStartTime <= wxAudioContext.currentTime) {
      callback();
    } else {
 ...
 */
function collectFrame(audioData) {
...
function archiveCurrentRound() {
  ...
  var totalLen = 0;
  for (var i = 0; i < currentRoundFrames.length; i++) {
    totalLen += currentRoundFrames[i].byteLength;
  }
  var merged = new Uint8Array(totalLen);
  var offset = 0;
  for (var _i = 0; _i < currentRoundFrames.length; _i++) {
    merged.set(new Uint8Array(currentRoundFrames[_i]), offset);
    offset += currentRoundFrames[_i].byteLength;
  }
...
function replayRandom() {
  ...
  var idx = audioHistory.length === 1 ? 0 : Math.floor(Math.random() * audioHistory.length);
  var pcmData = audioHistory[idx];
  console.log('[Player] Max复播第', idx + 1, '段,', (pcmData.byteLength / 1024).toFixed(1), 'KB');
  var int16 = new Int16Array(pcmData);
  ...
  if (wxAudioContext) {
    var buf = wxAudioContext.createBuffer(1, float32.length, 24000);
    buf.getChannelData(0).set(float32);
    replaySource = wxAudioContext.createBufferSource();
    replaySource.buffer = buf;
    var gain = wxAudioContext.createGain();
    gain.gain.value = 0.7;
...
function playPCMWeixin(audioData) {
  ...
  var sumSquares = 0;
  for (var i = 0; i < int16.length; i++) {
    var sample = int16[i] / 32768;
    float32[i] = sample;
    sumSquares += sample * sample;
  }
  var buffer = wxAudioContext.createBuffer(1, float32.length, 24000);
  buffer.getChannelData(0).set(float32);
  var source = wxAudioContext.createBufferSource();
  source.buffer = buffer;
  source.connect(wxGainNode);
  var now = wxAudioContext.currentTime;
  var startAt = Math.max(now, wxNextStartTime);
  source.start(startAt);
  wxNextStartTime = startAt + buffer.duration;

  // 用实际音频数据计算音量
  var rms = Math.sqrt(sumSquares / int16.length);
  ...
  if (!wxIsPlaying) {
    wxIsPlaying = true;
    wxVolumeTimer = setInterval(function () {
      if (!wxAudioContext || wxNextStartTime <= wxAudioContext.currentTime) {
        volumeLevel = 0;
